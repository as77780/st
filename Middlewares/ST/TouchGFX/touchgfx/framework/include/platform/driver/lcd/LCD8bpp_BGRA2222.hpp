...
protected:
    ...
    /**
     * @fn virtual void LCD8bpp_BGRA2222::drawTextureMapScanLine(const DrawingSurface& dest, const Gradients& gradients, const Edge* leftEdge, const Edge* rightEdge, const TextureSurface& texture, const Rect& absoluteRect, const Rect& dirtyAreaAbsolute, RenderingVariant renderVariant, uint8_t alpha, uint16_t subDivisionSize);
     *
     ...
     * @param bitsPerPixel    Bit depth of the glyph.0
...
private:
    ...
    FORCE_INLINE_FUNCTION uint32_t bgra2222to32bit(colortype col) const
    {
        return ((((int)col & 3) * 0x55) << 24) | (LCD8bpp_BGRA2222::getRedFromColor((col)) << 16) | (LCD8bpp_BGRA2222::getGreenFromColor(col) << 8) | LCD8bpp_BGRA2222::getBlueFromColor(col);
    }
